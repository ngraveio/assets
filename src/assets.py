...
class Asset:
    ...
    def save(self):
        if self.is_token:
            os.makedirs(
                f"{config.auto_arrange.output_folder}/{self.parent_id}/{config.auto_arrange.assets_folder}/{self.address}",
                exist_ok=True,
            )
            shutil.copyfile(
                self.icon,
                f"{config.auto_arrange.output_folder}/{self.parent_id}/{config.auto_arrange.assets_folder}/{self.address}/{config.auto_arrange.logo_name}.{self.icon_extension}",
            )
            self.is_saved = True
        else:
            os.makedirs(f"{config.auto_arrange.output_folder}/{self.id}", exist_ok=True)
            shutil.copyfile(
                self.icon,
                f"{config.auto_arrange.output_folder}/{self.id}/{config.auto_arrange.logo_name}.{self.icon_extension}",
            )
            self.is_saved = True
